...
```haskell
  layer(start, 0);
  layer(N₀, I₀), I₀ < I, edge(N₀, N₁), not (layer(N₁, I₁), I₁ < I) -> layer(N₁, I);
```
...
```haskell
  layer(start) = 0;
  loop I {
    layer(N₀, I₀), edge(N₀, N₁), not layer(N₁, _) -> layer(N₁, I);
  }
```
...
```haskell
  layer(start, 0);
  _NNNN_layer(start, 0, 0);

  layer(N₀, I₀), _NNNN_layer(N₀, I₀, _I₀), _I₀ < I,
  not (_NNNN_layer(N₁, _, _I₁), _I₁ < I),
  edge(N₀, N₁) ->
    layer(N₁, I), _NNNN_layer(N₁, I, I);
```
...
```haskell
  ...
  flow(U, V, N) += capacity(I) : path(U, V, I), I < N;

  -- Residual edges and their capacity after N steps
  edge(U, V, C), F = flow(U, V, N) - flow(V, U, N), C > F -> res_edge(U, V, N, C - F);

  -- Breadth-first search after N steps
  res_edge(source, V, N, _) -> res_path(source, V, N);
  res_path(_, V, N), res_edge(V, W, N, _), not (res_path(_, W, I), I < N) : W -> res_path(any(V), W, N);
